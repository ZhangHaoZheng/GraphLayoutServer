function multiScaleLayoutV3() { 
    ...
    this.transferIntoConnectedGraph = function() {
        ...
        for(var i = 0; i < this.groupArray.length - 1; i++) {
            ...
            this.addALink(index1, index2, this.groupArray[i].array, this.groupArray[i + 1].array);
        }
    };
    //complexity: O(|V|2), index1GroupindexArray include index1 and index2GroupindexArray include index2
    this.addALink = function(index1, index2, index1GroupindexArray, index2GroupindexArray) {
        for(var i = 0; i < index1GroupindexArray.length; i++) {
            for(var j = 0; j < index2GroupindexArray.length; j++) {
                var sIndex = index1GroupindexArray[i];
...
;        }
    ...
    this.modifyLayout = function() {
    	...
    	maxX = maxX - minX + this.displayAreaWidth / 20;
    	maxY = maxY - minY + this.displayAreaHeight / 20;
    	for(var i = 0; i < this.nodeArray.length; i++) {
    		this.nodeArray[i].x = (this.nodeArray[i].x - minX + this.displayAreaWidth / 20)
    						 * this.displayAreaWidth / maxX;
    		this.nodeArray[i].y = (this.nodeArray[i].y - minY + this.displayAreaHeight / 20)
